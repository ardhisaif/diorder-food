...
class IndexedDBService {
  ...
  async initDB(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        menuStore.createIndex('by_merchant', 'merchant_id', { unique: false });
        
        db.createObjectStore('cartItems', { keyPath: 'id', autoIncrement: true });
        db.createObjectStore('merchantInfo', { keyPath: 'id', autoIncrement: true });
      };
    });
  }
  ...
  async add<T extends keyof DBSchema>(storeName: T, item: DBSchema[T][number]): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  async update<T extends keyof DBSchema>(storeName: T, item: DBSchema[T][number]): Promise<void> {
    return new Promise((resolve, reject) => {
      const store = this.getStore(storeName, 'readwrite');
  ...
  async delete<T extends keyof DBSchema>(storeName: T, id: string | number): Promise<void> {
    return new Promise((resolve, reject) => {
      const store = this.getStore(storeName, 'readwrite');
